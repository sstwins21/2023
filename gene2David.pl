...
for $a (@data) {
  ...
  if($ARGV[1] eq "arg100") {
    ...
    $pop_count = "/scale_wlg_nobackup/filesets/nobackup/ga02470/article/chapter4/arg100/HEB/arg100_geneCount.txt";
    
  }elsif($ARGV[1] eq "acan") {
    $result = `grep -w $temp[0] "/scale_wlg_nobackup/filesets/nobackup/ga02470/annotation/berkley/acan_david.txt"`;
    $pop_count = "/scale_wlg_nobackup/filesets/nobackup/ga02470/article/chapter4/wasp_acan/new/acan_geneCount.txt"
    
  }elsif($ARGV[1] eq "cli") {
    $result = `grep -w $temp[0] "/scale_wlg_nobackup/filesets/nobackup/ga02470/annotation/berkley/david.txt"`;
    $pop_count = "/scale_wlg_nobackup/filesets/nobackup/ga02470/article/chapter4/wasp_cli/new/cli_geneCount.txt";
  }  
  ...
  if($temp2[1] ne "") {
    ...
    $count =~ s/\R//g;
    @temp3 = split(/\s+/, $count);
    $HEB{$temp2[1]}{$temp[1]} = $HEB{$temp2[1]}{$temp[1]} + $temp3[$pop_index];
...
foreach my $key (keys %HEB) {
  ...
  print "$key\t$HEB{$key}{$hoo}\t$HEB{$key}{$gei}\t$HEB{$key}{$bal}\t$final_HEB\t$final_ratio\n"; 
}
