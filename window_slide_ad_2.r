...
for(i in 1:18) {
  ...
  cli_data <- data[ which(data$V1==scaff), ]
  ...
  newDT <- data.frame(cli_win$pos_wind, cli_win$het_wind)
  #colnames(newDT) <- c("pos", "hookeri", "Acan", "Ances") 
  colnames(newDT) <- c("position", "C.hookeri")

  plot_name <- paste("axp_",chrom_num,".jpeg", sep= "")
  gg_name<- paste("scaffold",i,"_plot", sep= "")
  



  g <- ggplot(data = newDT, mapping = aes(x = position)) + geom_line(aes(y = C.hookeri)) + xlim(1, chrom$V2[i]) + ylim(0, 1) + theme_classic()
